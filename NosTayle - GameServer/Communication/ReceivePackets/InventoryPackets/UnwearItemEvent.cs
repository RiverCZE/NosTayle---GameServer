...
{
    internal sealed class UnwearItemEvent : Interface
    {
        public void Handle(Session Session, SessionMessage Event)
        {
            if (!Session.GetPlayer().inventory.isBlocked && Event.valuesCount == 2)
                Session.GetPlayer().inventory.UnWearItem(Session.GetPlayer(), Convert.ToInt32(Event.GetValue(0)), Convert.ToInt32(Event.GetValue(1)));
        }
    }
}
